...
const BlogContentsEdit = () => {
  ...
  useEffect(() => {
    fetch(
      `http://localhost/Projeler/PixelEvren/PixelEvren_BackEnd/Api/blog-get.php?id=${id}`,
    )
      .then((res) => res.json())
      .then((result) => {
        if (result.status === "success") {
          ...
          setİcerikBaslik(blog.icerikbaslik);
          setAciklama(blog.aciklama);
          setİcerik1(blog.icerik1);
          setİcerik2(blog.icerik2);
          setİcerik3(blog.icerik3);
          setİcerik4(blog.icerik4);
          setBlogSoz(blog.blogsoz);
          setSelectedCategory(blog.kategori);
          setResim(blog.resim);
        }
      })
...
export default BlogContentsEdit;

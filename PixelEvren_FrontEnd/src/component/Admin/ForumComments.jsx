...
const ForumComments = () => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          "http://localhost/Projeler/PixelEvren/PixelEvren_BackEnd/Api/get-admin-forum-comments.php",
        );
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(
      "http://localhost/Projeler/PixelEvren/PixelEvren_BackEnd/Api/delete-forum-comments.php",
      {
        method: "POST",
        body: formData,
      },
            ...
            {Array.isArray(comments) && comments.length > 0 ? (
              <table className="min-w-full border border-gray-300 bg-white rounded-lg shadow">
                <thead className="bg-gray-200">
                  <tr>
                    <th className="px-4 py-2 text-left">Forum İd</th>
                    <th className="px-4 py-2 text-left">Yorum</th>
                    <th className="px-4 py-2 text-left">Yorum Sahibi</th>
                    <th className="px-4 py-2 text-left">Beğeni</th>
                ...
                <tbody>
                  {comments.map((item) => (
                    ...
                    >
                      <td className="px-4 py-2">{item.forum_id}</td>

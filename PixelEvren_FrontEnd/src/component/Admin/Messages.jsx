...
const Messages = () => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          "http://localhost/Projeler/PixelEvren/PixelEvren_BackEnd/Api/get-mesaj.php",
        );
        const data = await res.json();

        const normalized = Array.isArray(data)
          ? data
          : Array.isArray(data?.mesaj)
            ? data.mesaj
            : [];
        setMesaj(normalized);
      } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(
      "http://localhost/Projeler/PixelEvren/PixelEvren_BackEnd/Api/delete-mesaj.php",
      {
        method: "POST",
        body: formData,
      },
        ...
        <main className="flex-1 p-6 md:p-10 bg-gray-100 overflow-x-hidden">
          <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            ...
            <p className="text-gray-600">
              Burada gelen mesajları düzenleyebilirsiniz.
            </p>
          </div>
          <div className="mt-8 overflow-x-auto">
            {Array.isArray(mesaj) && mesaj.length > 0 ? (
              <table className="min-w-full border border-gray-300 bg-white rounded-lg shadow">
                ...
                <tbody>
                  {mesaj.map((item) => (
                    ...
                    >
                      <td className="px-4 py-2">{item.mesaj}</td>

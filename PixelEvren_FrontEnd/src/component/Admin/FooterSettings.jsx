...
const FooterSettings = () => {
  ...
  const handleSave = async () => {
    ...
    formData.append("tanitim", tanitim);
    ...
    const response = await fetch(
      "http://localhost/Projeler/PixelEvren/PixelEvren_BackEnd/Api/footer.php",
      {
        method: "POST",
  ...
  useEffect(() => {
    fetch(
      "http://localhost/Projeler/PixelEvren/PixelEvren_BackEnd/Api/get-footer.php",
    )
      .then((res) => res.json())
      .then((data) => {
        if (!data) return;
        setBaslik(data.baslik);
        setTanitim(data.tanitim);
        ...
        <main className="flex-1 p-6 md:p-10 bg-gray-100 overflow-x-hidden">
          <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            ...
            <p className="text-gray-600">
              Burada footer ayarlarını düzenleyebilirsiniz.
            </p>
          </div>
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <input
                  type="text"
                  value={tanitim}
                  onChange={(e) => setTanitim(e.target.value)}
                  placeholder="Footer Tanıtımı..."
                  className="bg-gray-100 border border-gray-300 w-full px-3 py-2 rounded-md text-sm focus:ring-2 focus:ring-green-500 outline-none"
                />
              </div>
              {/* Social Link */}
